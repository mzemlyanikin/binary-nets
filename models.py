...
def reid_binary_resnet18(loss, num_classes, **kwargs): # paper, page 5, fig. 1(b)
    model = ReIdResNet(loss=loss, num_classes=num_classes, block=BinaryBasicBlock, layers=[2, 2, 2, 2], **kwargs)
    return model


def reid_parallel_resnet18_no_squeeze(loss, num_classes, **kwargs): # paper, page 5, fig. 2 (c)
    model = ReIdResNet(loss=loss, num_classes=num_classes, block=ParallelBinaryBasicBlockNoSqueeze, layers=[2, 2, 2, 2], **kwargs)
    return model


def reid_parallel_resnet18_with_squeeze(loss, num_classes, **kwargs): # paper, page 5, fig. 2 (b)
    model = ReIdResNet(loss=loss, num_classes=num_classes, block=ParallelBinaryBasicBlockWithSqueeze, layers=[2, 2, 2, 2], **kwargs)
    return model


def reid_parallel_resnet18_with_fusion_gate(loss, num_classes, **kwargs): # paper, page 6, fig. 3
    model = ReIdResNet(loss=loss, num_classes=num_classes, block=ParallelBinaryBasicBlockWithFusionGate, layers=[2, 2, 2, 2], parallel=2, **kwargs)
    return model
